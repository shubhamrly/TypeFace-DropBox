...
const getFiles = async (req, res) => {
  try {
    ...
    console.info(`Files found: ${files.length}, :  ${page} out of ${total}`);
  } catch (error) {
    console.error(
      "Something went wrong, check backend service for getFileController",
      error
    );
...
const previewFile = async (req, res) => {
  try {
    const { filePath, file } = await fileService.getFilePathAndMeta(
      req.params.filename
    );
    ...
    if (!res.headersSent) {
      ...
      res
        .status(500)
        .send({ error: "Something went wrong while loading the file preview" });
    }
  }
};
const renameFile = async (req, res) => {
  try {
    ...
    if (!newName || typeof newName !== "string" || newName.trim() === "") {
      return res.status(400).send({ error: "file name exists" });
    }
    ...
    console.info(`File renamed successfully: ${filename} to ${newName}`);
  } catch (error) {
...
const deleteFile = async (req, res) => {
  try {
    ...
    if (!deleted) {
      ...
      return res.status(404).send({ error: "file not found: ", filename });
    }
